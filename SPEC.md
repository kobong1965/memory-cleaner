...
8. 迷你模式由完整版 EXE 的参数模式提供；保留独立桌面快捷方式作为同一程序的快速入口，并继续保留原安装位置和完整版快捷方式。
...
11. 完整版与迷你悬浮版是同一个产品和同一个 `MemoryPilot.exe`；主程序通过 `--mini` 参数进入悬浮模式，完整版提供可访问的开关控制该模式。
...
- 顶部提供“迷你悬浮窗”开关；打开时启动同一个 `MemoryPilot.exe --mini`，关闭时通过命名事件请求悬浮模式正常退出。
- 开关每 500 毫秒同步一次实际悬浮模式状态；用户从悬浮窗菜单退出后，开关应自动恢复为关闭。
- 启动/关闭过渡期间禁用重复切换并显示明确文字，不因后台状态轮询闪动。
...
- 迷你模式由 `MemoryPilot.exe --mini` 直接承载；兼容入口可以继续调用该参数，但主程序不依赖独立的 `MemoryPilotMini.exe` 才能工作。
- 迷你模式监听同产品命名退出事件，收到完整版关闭开关的请求后正常结束 Qt 事件循环，不使用强制终止。
...
memory-pilot/
  ...
  src/memory_pilot/
    ...
    ui/
      mini_mode.py                # 同一 EXE 的迷你模式启动、状态检查与正常退出信号
...
- 迷你模式测试覆盖同一 EXE 启动命令、命名互斥状态、退出事件、开关过渡和外部退出后的状态回正。
...
- 完整版“迷你悬浮窗”开关可启动和关闭同一个 `MemoryPilot.exe` 的 `--mini` 模式；原迷你快捷方式也改为指向该主 EXE 参数模式。
